...
hw = HelloWorld("jan", "Geröll")
greeting = hw.helloWorld()
print(greeting)
...
for element in input:
    ...
    # horizontal
        ...
        if x1 < x2:
            diff = x2 - x1
            for i in range(diff + 1):
                grid[x1 + i][y1] = grid[x1 + i][y1] + 1
        # right left
        else:
            diff = x1 - x2
            for i in range(diff + 1):
                grid[x2 + i][y1] = grid[x2 + i][y1] + 1
    # diagonal
    else:
        # start top left
        if x1 < x2 and y1 < y2:
